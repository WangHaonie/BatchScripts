...
if (Get-Command Invoke-WebRequest -ErrorAction SilentlyContinue) { 
    ...
    Write-Host " "
    Write-Host "错误：PowerShell 命令 Invoke-WebRequest 不可用，将无法下载文件，脚本无法继续运行。" -ForegroundColor Red
}

########### 检查系统中是否存在 Aria2、Wget、cURL 以便加快文件下载速度 - 代码未完成 ##########
if (Get-Command -Name aria2c.exe -ErrorAction SilentlyContinue) {
    Write-Host "aria2c OK"
} else {
    Write-Host "aria2 NO"
}
if (Get-Command -Name wget.exe -ErrorAction SilentlyContinue) {
    Write-Host "wget OK"
} else {
    Write-Host "wget NO"
}
